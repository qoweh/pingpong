...
현재 기본 제어 모델은 `keep_v39_17d`다. 모델 파일 이름은 `keep_v39_17d_model.zip`이고, Stable-Baselines3 2.8.0의 PPO 형식으로 저장되어 있다. 초기 active model은 `.env`의 `PINGPONG_POLICY_MODEL_PATH`로 정하지만, 실행 중에는 웹 UI의 Model 섹션에서 `rl/artifacts` 아래의 다른 대표 PPO 모델로 전환할 수 있다.
...
웹 runtime은 모델을 `PPO.load(...)`로 읽고, 매 step `predict(observation, deterministic=True)` 형태로 action을 얻는다. deterministic inference에서는 학습 때처럼 action을 무작위로 샘플링하기보다 현재 policy가 고른 대표 action을 사용한다.
...
`GET /api/models`는 사용 가능한 모델 목록과 현재 active model을 반환한다. `POST /api/models/select`는 선택한 PPO zip을 로드하고 새 env kwargs, 공 시작값 config, policy metadata를 적용한다. 기존 websocket 세션은 생성 당시 runtime을 유지하고, 프론트엔드는 모델 전환 뒤 캔버스를 다시 연결해 새 세션을 시작한다.
